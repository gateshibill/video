	...
	<form class="layui-form layui-form-pane" action="">
			<div class="layui-form-item">
				<div class="layui-inline pane-input">
					<label class="layui-form-label">砍价ID</label>
					<div class="layui-input-block ">
						<input type="text" id="keyWords" name="keyWords"
							placeholder="请输入砍价ID" autocomplete="off" class="layui-input">
					</div>
				</div>
	 ...
	 var articleType = '${articleType}';
		...
		function getlist(keyWords) {
			tableIns = table.render({
				...
				url : '${pageContext.request.contextPath}/wx/activity/getAllCutGoodsList.do',
				cols : [ [ {
					field : 'id',
					title : '砍价ID',
					width : 80,
		...
		table.on('tool(Article)', function(obj) {
			...
			if (obj.event === 'editArticle') {
				var index = layui.layer.open({
					title : "编辑砍价商品",
					type : 2,
					content :'${pageContext.request.contextPath}/wx/activity/goUpdateCutGoodsById.do?id='+data.id,
				...
				layui.layer.full(index);
 				...
 				layer.confirm('确认删除该砍价商品，该过程不可逆', {icon: 3, title:'提示'}, function(index){
					  $.ajax({
						  url:'${pageContext.request.contextPath}/wx/activity/deleteCutGoodsById.do?id='+data.id,						
						  type:'post',
						  dataType:'json',
