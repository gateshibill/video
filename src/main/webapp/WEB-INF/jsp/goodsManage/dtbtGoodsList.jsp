	...
	<form class="layui-form layui-form-pane" action="">
			<div class="layui-form-item">
				<div class="layui-inline pane-input">
					<label class="layui-form-label">秒杀ID</label>
					<div class="layui-input-block ">
						<input type="text" id="keyWords" name="keyWords"
							placeholder="请输入秒杀ID" autocomplete="off" class="layui-input">
					</div>
				</div>
	 ...
	 var articleType = '${articleType}';
		...
		function getlist(keyWords) {
			tableIns = table.render({
				...
				url : '${pageContext.request.contextPath}/wx/activity/getAllDtbtGoodsList.do',
				cols : [ [ {
					field : 'id',
					title : '秒杀ID',
					width : 80,
		...
		table.on('tool(Article)', function(obj) {
			...
			if (obj.event === 'editArticle') {
				var index = layui.layer.open({
					title : "编辑秒杀商品",
					type : 2,
					content :'${pageContext.request.contextPath}/wx/activity/goUpdateDtbtGoodsById.do?id='+data.id,
				...
				layui.layer.full(index);
 				...
 				layer.confirm('确认删除该秒杀商品，该过程不可逆', {icon: 3, title:'提示'}, function(index){
					  $.ajax({
						  url:'${pageContext.request.contextPath}/wx/activity/deleteDtbtGoodsById.do?id='+data.id,						
						  type:'post',
						  dataType:'json',
