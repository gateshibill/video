...
<blockquote class="layui-elem-quote">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="consignee" id="consignee"
								placeholder="请输入退货人姓名" class="layui-input">
					</div>
				</div>
	...
	<a class="layui-btn layui-btn-primary layui-btn-mini" lay-event="dealOrder">确认退款</a>
...
<script type="text/html" id="payTypeTpl">
    ...
    {{# } else if(payType == 2){ }}
			<span style="color:green">余额支付</span>
    {{# } else if(payType == 3){ }}
		    <span style="color:green">积分兑换</span>
	{{# } else if(payType == 4){ }}
	...
	<script>
		...
		table.on('tool(descovery)', function(obj) {
			...
			}else if(obj.event === 'dealOrder'){
				if(data.payType == 4){
					layer.confirm('确认退款?', {
						btn : [ '确认', '取消' ]
					}, function() {
						$.post('${ctx}/userorder1/aliRefund.do', {
							'orderNo' : data.orderNo,  //用订单号退款增强安全性
						}, function(data) {
							if (data.code == 0) {
								layer.msg("退款成功!");
					...
					layer.confirm('确认退款?', {
						...
						$.post('${ctx}/userorder1/wxrefund.do', {
							'orderNo' : data.orderNo,  //用订单号退款增强安全性
						}, function(data) {
							if (data.code == 0) {
								layer.msg("退款成功!");
