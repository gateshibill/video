			...
			<div class="layui-form-item">
			   <div class="layui-inline pane-input">
				...
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<select id="payType" class="search-input" name="orderStatus"
								lay-verify="required" lay-search="">
								<option value="" selected="selected">全部</option>
...
<script type="text/html" id="deskId">
     {{# var deskId=d.deskId}}
    {{# if (deskId > 0){}}
          {{deskId}} 
    {{# } else{ }}
...
<script type="text/html" id="payTypeTpl">
	...
	<script>
		...
		function doSearch() {
			...
			getlist(plat,status,name,dtr,payType);
		}

		function getlist(plat,status,name,dtr,payType) {
		...
		table.on('tool(descovery)', function(obj) {
			...
			if (obj.event === 'descoveryDetail') {
				...
				var index = layui.layer.open({
					title : "编辑订单",
					type : 2,
					content : "${ctx}/userorder1/userOrderDateils.do?orderID="+data.orderID,
