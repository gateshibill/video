  ...
  <a class="layui-btn layui-btn-danger layui-btn-mini" lay-event="delcontent">删除</a>
	...
	<script>
		...
		function doSearch() {
			...
			getlist(userId,contentTitle);
		}
		
		function getlist(userId,contentTitle) {
			tableIns = table.render({
				...
				url : '${ctx}/shareContract/showContractList.do',
				cols : [ [ {
					field : 'contentId',
					title : 'ID',
					width : 100,
		...
		$(window).one("resize",function(){
			$(".newsAdd_btn").click(function(){
				var index = layui.layer.open({
					title : "添加合约内容",
					type : 2,
					content : "${ctx}/shareContract/addContent.do",
...
table.on('tool(customer)', function(obj) {
			var data = obj.data;
			if (obj.event === 'editcontent') {
				 
				var index = layui.layer.open({
					title : "编辑共享内容",
					type : 2,
					content : "${ctx}/shareContract/goupdatecontent.do?contentId="+data.contentId+"&contentTitle="+data.contentTitle,
					success : function(layero, index){
						setTimeout(function(){
							layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
								tips: 3
			...
			} else if (obj.event === 'delcontent') { //删除操作
				if(data.usedCount > 0){
				...
				layer.confirm('确定删除该条数据', {
					  btn: ['确定','取消'] //按钮
						...
						$.ajax({
							 url:"${ctx}/shareContract/delcontent.do",
							 type:'post',
							 dataType:'json',
