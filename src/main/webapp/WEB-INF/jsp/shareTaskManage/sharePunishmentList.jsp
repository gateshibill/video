...
<blockquote class="layui-elem-quote news_search">
	<form class="layui-form layui-form-pane" method="post" action="" id="listform">
		<div class="layui-inline">
			<label class="layui-form-label">惩罚标题</label>	
...
<a class="layui-btn layui-btn-mini" lay-event="editpunish">编辑</a>
  <a class="layui-btn layui-btn-danger layui-btn-mini" lay-event="delpunish">删除</a>
</script>
	...
	<script>
		...
		function doSearch() {
			...
			var punishmentContent = $("#keyword").val();
			getlist(punishmentId,punishmentContent);
		}
		
		function getlist(punishmentId,punishmentContent) {
			tableIns = table.render({
				...
				url : '${ctx}/shareContract/showPunishmentList.do',
				cols : [ [ {
					field : 'punishmentId',
					title : 'ID',
					width : 100,
		...
		$(window).one("resize",function(){
			$(".newsAdd_btn").click(function(){
				var index = layui.layer.open({
					title : "添加惩罚",
					type : 2,
					content : "${ctx}/shareContract/addPublisher.do",
		...
		table.on('tool(customer)', function(obj) {
			var data = obj.data;
			if (obj.event === 'editpunish') {
				 
				var index = layui.layer.open({
					title : "编辑共享惩罚",
					type : 2,
					content : "${ctx}/shareContract/goupdatepunish.do?punishmentId="+data.punishmentId+"&punishmentContent="+data.punishmentContent,
					success : function(layero, index){
						setTimeout(function(){
							layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
								tips: 3
			...
			} else if (obj.event === 'delpunish') { //删除操作
				var punishmentId = data.punishmentId;
				if(data.usedCount > 0){
				...
				layer.confirm('确定删除该条数据', {
					  btn: ['确定','取消'] //按钮
						...
						$.ajax({
							 url:"${ctx}/shareContract/delpunish.do",
							 type:'post',
							 dataType:'json',
