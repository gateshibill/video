...
<body>
	...
	<form class="layui-form layui-form-pane" method="post" action="" id="listform">
		...
		<div class="layui-inline">
		    ...
		    <div class="layui-input-inline">
		    	<div style="float:left;">
					<select id="noticeType" class="layui-input search_input" name="noticeType" lay-search="">
					   <option value="">全部</option>
					   <option value="1">语音消息接收</option>	
					   <option value="2">服务号模板消息接收</option>					  			   
					 </select>
				</div>
		    </div>
		</div>
	...
	<script>
		...
		function getlist(pid) {
			tableIns = table.render({
				...
				url : '${ctx}/pushnotice/getpushnoticelist.do',
				cols : [ [ 
// 				 {
// 					checkbox : true,
// 					LAY_CHECKED : false,
...
// 				}, 
		...
		$(window).one("resize",function(){
			$(".newsAdd_btn").click(function(){
				...
				var index = layui.layer.open({
					title : "添加消息接收信息",
					type : 2,
					content : "${ctx}/pushnotice/goaddnotice.do?loginPlat="+loginPlat+"&noticeType="+noticeType,
		...
		table.on('tool(affiche)',function(obj) {
							...
							/* 	var index = layui.layer.open({
								...
								layer.confirm('您确认删除该条信息', {
								    btn: ['确认','取消'], 
								    shade: false //不显示遮罩
								}, function(){
								     $.ajax({
								    	 url:"${ctx}/pushnotice/delnotice.do",
								    	 type:"post",
								    	 dataType:"json",
