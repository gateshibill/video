...
<blockquote class="layui-elem-quote news_search">
  ...
  <a class="layui-btn layui-btn-mini" lay-event="edittaskreason">编辑</a>
  <a class="layui-btn layui-btn-danger layui-btn-mini" lay-event="deltaskreason">删除</a>
	...
	<script>
		...
		function getlist(contentTitle) {
			tableIns = table.render({
				...
				url : '${ctx}/sharetask/taskreasonlist.do',
				cols : [ [ {
					field : 'reasonId',
					title : 'ID',
					width : 100,
		...
		$(window).one("resize",function(){
			$(".newsAdd_btn").click(function(){
				var index = layui.layer.open({
					title : "添加任务原因",
					type : 2,
					content : "${ctx}/sharetask/addtaskreason.do",
...
table.on('tool(customer)', function(obj) {
			...
			if (obj.event === 'edittaskreason') {
				var index = layui.layer.open({
					title : "编辑任务原因",
					type : 2,
					content : "${ctx}/sharetask/updatetaskreason.do?reasonId="+data.reasonId,
					success : function(layero, index){
						setTimeout(function(){
							layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
								tips: 3
				...
				layui.layer.full(index);
			} else if (obj.event === 'deltaskreason') { //删除操作
				var reasonId = data.reasonId;
				layer.confirm('确定删除该条数据', {
					  btn: ['确定','取消'] //按钮
					}, function(){
						$.ajax({
							 url:"${ctx}/sharetask/deltaskreason.do",
							 type:'post',
							 dataType:'json',
