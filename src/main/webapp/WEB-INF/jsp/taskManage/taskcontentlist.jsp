...
<a class="layui-btn layui-btn-mini" lay-event="edittaskcontent">编辑</a>
  <a class="layui-btn layui-btn-danger layui-btn-mini" lay-event="deltaskcontent">删除</a>
</script>
	...
	<script>
		...
		function getlist(contentTitle) {
			tableIns = table.render({
				...
				url : '${ctx}/sharetask/taskcontentlist.do',
				cols : [ [  {
					field : 'contentId',
					title : 'ID',
					width : 100,
		...
		$(window).one("resize",function(){
			$(".newsAdd_btn").click(function(){
				var index = layui.layer.open({
					title : "添加任务内容",
					type : 2,
					content : "${ctx}/sharetask/addtaskcontent.do",
		...
		table.on('tool(customer)', function(obj) {
			...
			if (obj.event === 'edittaskcontent') {
				...
				var index = layui.layer.open({
					title : "编辑任务内容",
					type : 2,
					content : "${ctx}/sharetask/updatetaskcontent.do?contentId="+data.contentId+"&contentText="+data.contentText,
					success : function(layero, index){
						setTimeout(function(){
							layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
								tips: 3
			...
			} else if (obj.event === 'deltaskcontent') { //删除操作
