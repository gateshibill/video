...
<body>
	<div style="margin: 15px;">
		<fieldset class="layui-elem-field layui-field-title"
			...
			<legend>技能【${common.descoveryTitle}】的详情</legend>
		</fieldset>
		<div class="layui-form">
			...
			<div class="layui-form-item">
				<label class="layui-form-label">技能相册：</label>
				<div class="layui-input-block">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<label class="detail-label-value"><fmt:formatDate
							value="${common.startTime}" type="both" />-
						<fmt:formatDate
							value="${common.endTime}" type="both" /></label>
				</div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label">服务费用：</label>
				<div class="layui-input-block">
					<label class="detail-label-value">${common.servicePrice}</label>
				</div>
			</div>
			<div class="layui-form-item">
			...
			<div class="layui-form-item">
				<label class="layui-form-label">附赠服务：</label>
				<div class="layui-input-block">
					<label class="detail-label-value"> 
					<c:forEach items="${skillTags}" var="tags">
							<c:if test="${tags.childtagOrder==1}">
								<c:choose>
									<c:when
										test="${fn:contains(common.getExplanatoryService(),tags.getChildtagId())}">
										<input type="checkbox" name="explanatoryService"
											id="explanatoryService" value="${tags.getChildtagId()}"
											lay-verify="explanatoryService" title="${tags.getChildtagName()}"
											checked="checked"> 
								</c:when>
									<c:otherwise>
										<input type="checkbox" name="explanatoryService"
					...
					<label class="detail-label-value">
						<c:forEach items="${skillTags}" var="tags">
							<c:if test="${tags.childtagOrder==2}">
								<c:choose>
									<c:when
										test="${fn:contains(common.getServiceWay(),tags.getChildtagId())}">
										<input type="checkbox" name="explanatoryService"
											id="explanatoryService" value="${tags.getChildtagId()}"
											lay-verify="explanatoryService" title="${tags.getChildtagName()}"
											checked="checked"> 
			...
			<div class="layui-form-item">
				<label class="layui-form-label">技能详情：</label>
				<div class="layui-input-block">
