...
<body>
		...
		<blockquote class="layui-elem-quote news_search">	
				<div class="layui-inline">
					...
					<div class="layui-input-block">
						<input type="text" name="ticketName" id="ticketName" value="${ticket.ticketName}"
								placeholder="请输入卡券名称" class="layui-input">
					</div>
				</div>
  ...
  <a class="layui-btn layui-btn-mini" lay-event="fenToken">分发</a>
  <a class="layui-btn layui-btn-mini" lay-event="ticketRecord">记录</a>
  <a class="layui-btn layui-btn-danger layui-btn-mini" lay-event="delToken">删除</a>
</script>
<script type="text/html" id="ticketTypeTpl">
    {{# var ticketType=d.ticketType}}
    {{# if (ticketType==1){}}
   		立减券
	...
	<script>
		...
		function getlist(ticketName) {
			tableIns = table.render({
				...
				url : '${ctx}/cardticket/getCardTicketList.do',
				cols : [ [ {
					field : 'ticketId',
					title : 'ID',
					width : 100,
						...
						function(obj) {
							...
							if (obj.event === 'editToken') {
								...
								layui.layer.full(index);
								 ...
								 alert("暂未开发")
							}else if(obj.event === 'ticketRecord'){
								var index = layui.layer
								.open({
									title : "卡券记录",
									type : 2,
									content : "${ctx}/cardticket/userTicketIndex.do?ticketId="
											+ data.ticketId,
									success : function(layero, index) {
										setTimeout(
												function() {
