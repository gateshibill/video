	...
	<form class="layui-form" method="post" action="" id="listform">
		<blockquote class="layui-elem-quote news_search">
			...
			<div class="layui-inline">
				...
				<div class="layui-input-inline">
					<div style="float: left;">
						<select id="advType" class="layui-input search_input"
							name="advType" lay-search="">
							<option value="">管理人员</option>
							<option  value="1" <c:if test="${adviser.advType == 1}">selected=""</c:if> >顾问</option>
							<option  value="2" <c:if test="${adviser.advType == 2}">selected=""</c:if>>设计师</option>		
							<option  value="3" <c:if test="${adviser.advType == 3}">selected=""</c:if>>工长</option>		
						</select>
					</div>
				</div>
			</div>
...
<script type="text/html" id="advTypeTl">
{{# var advType =d.advType}}
{{# if (advType==1){}}
		<span style="color:green;">顾问</span>
{{# }else if(advType==2){ }}
		<span style="color:red;">设计师</span>
{{# } else if(advType==3){ }}
		<span style="color:black;">工长</span>	
{{# } }}
</script>	
	...
	<script>
		...
		function doSearch() {
			...
			getlist(Recommend,advType);
		}

		function getlist(Recommend,advType) {
			tableIns = table.render({
				...
				url : '${ctx}/adviser/getAdviserList.do',
				cols : [ [ {
					field : 'id',
					title : 'ID',
					width : 100,
									...
									.click(
										...
										var index = layui.layer
											.open({
												title : "添加顾问",
												type : 2,
											content : "${ctx}/adviser/addAdviser.do",
										success : function(
											layero,
											index) {
										setTimeout(
							...
							if (obj.event === 'editToken') {
								var index = layui.layer
										.open({
											title : "编辑顾问",
											type : 2,
											content : "${ctx}/adviser/updateAdviser.do?id="
													+ data.id,
