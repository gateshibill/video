		...
		<div class="layui-form">
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<input type="text" name="homeTypeName" id="homeTypeName" autocomplete="off" class="layui-input" lay-verify="homeTypeName" value="${hType.homeTypeName}">
				</div>
			</div>
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input type="number" name="orderStatus"  autocomplete="off"
						class="layui-input" lay-verify="orderStatus" value="${hType.orderStatus}" style="width:135px;">
				</div>
			</div>
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" style="width:135px;">
					<select name="isShow" lay-verify="isShow" style="width:135px;">
						<option value="0" <c:if test="${hType.isShow==0}">selected="selected"</c:if>>否</option>
						<option value="1" <c:if test="${hType.isShow==1}">selected="selected"</c:if>>是</option>
					</select>
				</div>
			</div>	
			<div class="layui-form-item">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<textarea placeholder="请输入详情" name="content" class="layui-textarea" lay-verify="content">${hType.content}</textarea>
				</div>
			</div>
...
//保存图片
var designArr = [];
var designArr_1 = '${hType.homeImageList}';
if(designArr_1){
	 ...
	 if(designArr.length>0){
		for(var i = 0; i < designArr.length; i++){
			//添加项
			$('#design-xiang').append('<ul class="layer-photos-demo flex-box" id="design-Images" lay-verify=""><p style="padding:10px;">图片名：'+designArr[i].name+'</p></ul>');
			//遍历
...
//封面回显
var collCoverArr="${hType.homeCoverImage}".split(",");
if(collCoverArr != ""){
	...
	var form = layui.form, layer = layui.layer;
		...
		form.on('submit(demo1)', function(data) {
			if(designArr.length < 1){
				...
				$.ajax({
					type : "post",
					url : "${ctx}/homeType/doupdateHomeType.do",
					dataType:"json",
					data : {
						"id":"${hType.id}",
