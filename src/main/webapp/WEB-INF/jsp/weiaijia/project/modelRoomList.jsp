				...
				<div class="layui-input-inline">
					<div style="float: left;">
						<select id="workType" class="layui-input search_input"
							...
							<option  value="1" <c:if test="${pwork.workType == 1}">selected="selected"</c:if>></option>
							<option  value="2" <c:if test="${pwork.workType == 2}">selected="selected"</c:if>></option>		
						</select>
					</div>
				</div>
			</div>
	...
	<script>
		...
		function getlist(workType) {
			tableIns = table.render({
				...
				url : '${ctx}/newProject/getModelRoomList.do',
				cols : [ [ {
					field : 'id',
					title : 'ID',
					width : 100,
		...
		function() {
			$(".newsAdd_btn").click(
						...
						var index = layui.layer.open({
								title : "添加样板间",
								type : 2,
							content : "${ctx}/newProject/goAddModelRoom.do",
						success : function(
							layero,
							index) {
						setTimeout(
...
function(obj) {
	...
	if (obj.event === 'editToken') {
		var index = layui.layer.open({
			title : "编辑样板间",
			type : 2,
			content : "${ctx}/newProject/goUpdateModelRoom.do?id="
					+ data.id,
	...
	function() {
		$.post(
			'${ctx}/newProject/deleteModelRoom.do',
			{
				'id' : data.id
			},
			function(data) {
