		...
		<div class="layui-form">
			<div class="layui-form-item">
				<label class="layui-form-label">标题:</label>
					<div class="layui-input-block">
						<input type="text" name="workTitle" autocomplete="off" class="layui-input" lay-verify="workTitle" maxlength="255" value="${process.workTitle}">
					</div>
			</div>
			...
			<div class="layui-from-item">
				...
				<div class="layui-input-block">
					<select name="workType" lay-verify="workType" style="width:200px;height: 30px">
						<option value="1" <c:if test="${process.workType==1}">selected="selected"</c:if>>施工流程</option>
						<option value="2" <c:if test="${process.workType==2}">selected="selected"</c:if>>工艺标准</option>
					</select>
				</div> 
			</div>
			...
			<div class="layui-form-item" style="margin-top:10px;">
				<label class="layui-form-label">排序:</label>
					<div class="layui-input-block">
						<input type="number" name="orderStatus" autocomplete="off" class="layui-input" lay-verify="orderStatus" value="${process.orderStatus}">
					</div>
			</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<textarea placeholder="请输入施工描述" id="workDesc" lay-verify="workDesc" class="layui-textarea" name="workDesc" maxlength="450" id="workDesc">${process.workDesc}</textarea>
				</div>
			</div>
...
//保存图片
var designArr = [];
var designArr_1 = '${process.workImagesStr}';
if(designArr_1){
	 ...
	 if(designArr.length>0){
		for(var i = 0; i < designArr.length; i++){
			//添加项
			$('#design-xiang').append('<ul class="layer-photos-demo flex-box" id="design-Images" lay-verify=""><p style="padding:10px;">介绍：'+designArr[i].name+'</p></ul>');
			//遍历
	...
	var form = layui.form, layer = layui.layer;
		...
		form.on('submit(demo1)', function(data) {
			if(designArr.length < 1){
				...
				$.ajax({
					type : "post",
					url : "${ctx}/project/doUpdateAddProjectWorkProcess.do",
					dataType:"json",
					data : {
