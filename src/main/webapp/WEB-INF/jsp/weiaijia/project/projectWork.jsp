...
<body>
	<form class="layui-form" method="post" action="" id="listform">
		<blockquote class="layui-elem-quote news_search">
			<div class="layui-inline">
				...
				<a class="layui-btn layui-btn-normal newsJin_btn">施工进度</a>
			</div>
		</blockquote>
	</form>
  ...
  <a class="layui-btn layui-btn-mini" lay-event="findMinWork">查看小类</a>
...
<script>
		...
		function getlist(isRecommend,collType) {
			tableIns = table.render({
				...
				url : '${ctx}/newProject/getProjectBigWorkPlan.do',
				cols : [ [ {
					field : 'id',
					title : 'ID',
					width : 100,
					...
					field : 'bigPlanStartTimeS',
					title : '计划开始时间',
					width : 200,
					align : 'center',
					sort : true
				},{
					field : 'bigPlanEndTimeS',
					title : '计划结束时间',
					width : 200,
					align : 'center',
					sort : true
				}, {
					field : 'bigRealStartTimeS',
					title : '真实开始时间',
					width : 200,
					align : 'center',
					sort : true
				},{
					field : 'bigRealEndTimeS',
					title : '真实结束时间',
					width : 200,
					align : 'center',
					sort : true
					...
					$(".newsAdd_btn").click(function() {
								var index = layui.layer
										.open({
											title : "添加大类施工计划",
											type : 2,
											content : "${ctx}/newProject/goAddWorkBigPlan.do?projectId="+"${project.id}",
											success : function(
													layero,
													index) {
												setTimeout(
						...
						$(".newsJin_btn").click(function() {
								var index = layui.layer
										.open({
											title : "查看施工进度",
											type : 2,
											content : "${ctx}/newProject/goWorkJindu.do?projectId="+"${project.id}",
											success : function(
													layero,
													index) {
												setTimeout(
							...
							if (obj.event === 'editToken') {
								var index = layui.layer
										.open({
											title : "编辑大类施工计划",
											type : 2,
											content : "${ctx}/newProject/goUpdateWorkBigPlan.do?id="
													+ data.id,
								...
								.open({
									title : "小类施工计划",
									type : 2,
									content : "${ctx}/newProject/goProjectMinWork.do?planTypeId="
								...
								layer.confirm(
										...
										function() {
											$
													.post(
															'${ctx}/newProject/deleteWorkBigPlan.do',
															{
																'id' : data.id
															},
															function(data) {
